...
BUILDER_PROMPT = (
    ...
    "HEALTH ENDPOINT CONTRACT: This is a HARD REQUIREMENT. If the project has a "
    "/health endpoint (check SPEC.md), it must actively verify every external dependency "
    "the app connects to \u2014 storage, databases, caches, queues, etc. /health must call "
    "each dependency (e.g. BlobServiceClient.GetProperties(), database ping) and return "
    "per-check status. When a task adds or wires up a new external service client, you "
    "MUST also add a health check for that service to /health in the SAME task. Do not "
    "leave /health returning a static 200 while the app has real dependencies \u2014 that is "
    "a broken contract.\n\n"
    "Read your "
    ...
    "MILESTONE VERIFICATION: For a server or web app \u2014 start any required dependencies "
    "(emulators, databases) that the app connects to, then start the app, then curl "
    "/health. The /health endpoint must actually touch every external resource \u2014 make a "
    "real call to storage, database, cache, etc. \u2014 and report each one as connected. "
    "You are NOT done until /health returns HTTP 200 with all checks passing. If /health "
    "reports any check as unhealthy, or the app fails to start, or /health is missing "
    "checks for dependencies the app uses, diagnose and fix it before finishing the "
    "milestone. For a CLI tool, run it with a basic command (e.g. --help) and "
    "confirm it exits cleanly. For a library, confirm the main module imports without "
    "errors. If verification fails, fix it \u2014 commit the fix with a descriptive message "
    "and push. Stop the app and any dependencies you started before finishing. "
    "Once /health passes with all dependencies verified, you are done for this session."
)
