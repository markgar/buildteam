...
from buildteam.git_helpers import parse_ls_remote_output
...
def test_save_load_reviewer_checkpoint_per_builder(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("buildteam.sentinel.resolve_logs_dir", lambda: str(tmp_path))
...
def test_are_agents_idle_with_reviewer_numbered_logs(tmp_path, monkeypatch):
    ...
    from buildteam.sentinel import are_agents_idle
...
def test_branch_commit_prompt_has_branch_name_placeholder():
    from buildteam.prompts import REVIEWER_BRANCH_COMMIT_PROMPT
...
def test_branch_batch_prompt_has_branch_name_placeholder():
    from buildteam.prompts import REVIEWER_BRANCH_BATCH_PROMPT
...
def test_branch_prompts_can_be_formatted():
    ...
    from buildteam.prompts import REVIEWER_BRANCH_COMMIT_PROMPT, REVIEWER_BRANCH_BATCH_PROMPT
...
def test_extract_milestone_label_standard_branch():
    from buildteam.watcher import _extract_milestone_label
